...
class Indentation:
  ...
  def OliverPharrMethod(self, S, P, h):
    ...
    The following equations are used in that order:
      ...
      A = h_c(prefactors)   # Nicole: whether A = h_c makes misunderstanding. maybe A = f(h_c)
  ...
  def inverseOliverPharrMethod(self, S, P, E):
    ...
    h = h_c + self.beta*P/S
    return h.flatten() # Nicole: I have problems about the whole function, why h is calcultated twice according to h_c0 and h_cGuess? and Why h is expanded to multi dimensional and need to be flattened to one dimension?
  ...
  def calcStiffness2Force(self, minDepth=0.01, plot=True, calibrate=False):
    ...
    """
    compliance0 = self.compliance  # Nicole: compliance is not defined before??
    prefactors = None
