...
import math, io, re, os     # pylint: disable=multiple-imports
...
class Indentation:
  ...
  def YoungsModulus(self, modulusRed, nuThis=-1):
    ...
    if nuThis>0:
      numat = nuThis
    modulus = (1.0-numat*numat) / ( 1.0/modulusRed - (1.0-self.nuIndent*self.nuIndent)/self.EIndent )     # Pylint error: modulusRes is undefined
    return modulus
  ...
  def ReducedModulus(self, modulus, nuThis=-1):
    ...
    Args:
       Modulus (float): Young's modulus [GPa]
    ...
    if nuThis>0:
      numat = nuThis
    modulusRed =  1.0/(  (1.0-numat*numat)/modulus + (1.0-self.nuIndent*self.nuIndent)/self.EIndent )
    return modulusRed
  ...
  def StiffnessFromUnloading(self, p, h, plot=False):
    ...
    for cycleNum, cycle in enumerate(self.iLHU):
      ...
      try:
        opt, _ = curve_fit(self.UnloadingPowerFunc, h[mask],P[mask],    # pylint warning: Possible unbalanced tuple unpacking with sequence defined at line 837 of scipy.optimize.minpack: left side has 2 label(s), right side has 5 value(s) (308:8) [unbalanced-tuple-unpacking]
      ...
      except:       # pylint warning: No exception type(s) specified (318:6) [bare-except]
        if self.verbose>0:
      ...
      Stiffness0 = b*m*math.pow( (h[unloadStart]-hf), m-1)
      Stiffness.append(Stiffness0)
      validMask[unloadStart]=True
      if plot:
        plt.plot(h[mask],   self.UnloadingPowerFunc(h[mask],b,hf,m),'m-')
        StiffnessNew= p[unloadStart]-Stiffness0*h[unloadStart]
        plt.plot(h[mask],   Stiffness0*h[mask]+StiffnessNew, 'r--', lw=3)
    if plot:
      plt.xlim(left=0)
      plt.ylim(bottom=0)
  ...
  def popIn(self, correctH=True, plot=True, removeInitialNM=2.):
    ...
    depthRate = (h[1:]-h[:-1])
    x        = np.arange(len(depthRate))
    fits      = np.polyfit(x,depthRate,2)  #substract 2nd order fit b/c depthRate increases over time
    depthRate-= np.polyval(fits,x)
    ...
    def funct(depth, prefactor, h):
      diff           = depth-h
      if type(diff)==np.float64:   # pylint convention: Use isinstance() rather than type() for a typecheck. (384:9) [unidiomatic-typecheck]
        diff = max(diff,0.0)
      else:
        diff[diff<0.0] = 0.0
  ...
  def calcYoungsModulus(self, minDepth=-1, plot=False):
    ...
    if minDepth>0:
      #StiffnessAve = np.average(       self.modulusRed[ self.h>minDepth ] )
      StiffnessAve = np.average( modulus[  np.bitwise_and(modulus>0, self.h[self.valid]>minDepth) ] )
      StiffnessStd = np.std(     modulus[  np.bitwise_and(modulus>0, self.h[self.valid]>minDepth) ] )
      print("Average and StandardDeviation of Young's Modulus",round(StiffnessAve,1) ,round(StiffnessStd,1) ,' [GPa]')
    else:
      StiffnessAve, StiffnessStd = -1, 0
    if plot:
      h = self.h[self.valid]
      ...
      if minDepth>0:
        plt.axhline(StiffnessAve, color='k')
        plt.axhline(StiffnessAve+StiffnessStd, color='k', linestyle='dashed')
        plt.axhline(StiffnessAve-StiffnessStd, color='k', linestyle='dashed')
        plt.ylim([StiffnessAve-4*StiffnessStd,StiffnessAve+4*StiffnessStd])
      plt.xlabel(r'depth [$\mathrm{\mu m}$]')
      plt.ylim(ymin=0)
  ...
  def calcStiffness2Force(self, minDepth=0.01, plot=True, calibrate=False):
    ...
    def errorFunction(compliance):
      ...
      h_ = h[ h>minDepth ]
      Stiffness2load0  = Stiffness2load[ h>minDepth ]
      if len(h_)>4:
        prefactors = np.polyfit(h_,Stiffness2load0,1)
  ...
  def analyseDrift(self, plot=True, fraction=None, timeStart=None):
    ...
    if self.vendor == Vendor.Hysitron and self.fileName.endswith('.hld'):
      ...
      rate[:] = np.nan
      for idxEnd,t0 in enumerate(t):
        if t<20: continue
        idxStart = np.argmin(np.abs( t[:]-(t0-20.) )  )
  ...
  def plot(self, saveFig=False, show=True):
    ...
    if self.method != Method.CSM:
      ...
      ax.plot(self.hc, np.zeros_like(self.hc),"ob", label="h_c", markersize=10)
      if len(self.hc)<2:
  ...
  def calibration(self,eTarget=72.0,numPolynomial=3,critDepth=1.0,critForce=0.01,plotStiffness=False,plotTip=False, **kwargs):
    ...
    if numPolynomial is None:
      ...
      hc0 = np.logspace(np.log(0.0001),np.log(np.max(output[0,:])),num=30,base=np.exp(1))
      Ac0 = interpolationFunct(hc0)
      interpolationFunct = interpolate.interp1d(hc0, Ac0)
      del output, data
    else:
  ...
  def calibrateStiffness(self,critDepth=1.0,critForce=0.0001,plotStiffness=True, returnAxis=False):
    ...
    if plotStiffness:
      ...
      x0 = np.linspace(0, np.max(x)*1.1, 50)
      y0 = np.polyval(param, x0)
      ax.plot(x0,y0,'w-')
      ax.plot(x0,y0,'C0--')
  ...
  def verifyOneData(self):
    ...
    Stiffness = self.YoungsModulus(modulusRed)
    print("      Youngs Modulus [GPa] =",round(Stiffness[0],4),"  with error=", round((Stiffness[0]-190.257729329881)*100/190.257729329881,4),'%')
